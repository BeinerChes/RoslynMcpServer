...
Flow:
1. roslyn_get_instructions("plan") generates a token and writes to ~/.claude/roslyn-plan-token-{hash}
2. This hook reads the token and validates it via HTTP endpoint
3. If valid (< 1 min old), gh commands are allowed
"""
import sys
import json
import re
...
def get_plan_token(solution_path):
    """Read the plan token from the per-solution token file."""
    solution_hash = get_solution_hash(solution_path)
    token_file = os.path.join(os.path.expanduser('~'), '.claude', f'roslyn-plan-token-{solution_hash}')
    try:
