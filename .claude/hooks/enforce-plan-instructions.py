...
Flow:
1. roslyn_get_instructions("plan") generates a token and writes to .roslyn-mcp/plan-token
...
def find_roslyn_mcp_dir(start_dir=None):
    """Find the .roslyn-mcp directory by walking up from the start directory."""
...
def main():
    ...
    roslyn_dir = find_roslyn_mcp_dir()
    if not roslyn_dir:
        print('BLOCKED: No .roslyn-mcp directory found.', file=sys.stderr)
        print('Run: roslyn_get_instructions(topic: "plan")', file=sys.stderr)
        sys.exit(2)

    # Get the token
    token = get_token(roslyn_dir, 'plan')
    if not token:
