#!/usr/bin/env python3
"""
Hook: Enforce calling GetInstructions("git") before git commit/push.
...
Flow:
1. GetInstructions("git") generates a token and writes to .roslyn-mcp/git-token
...
def main():
    ...
    if not roslyn_dir:
        ...
        print('Run: GetInstructions(topic: "git")', file=sys.stderr)
        sys.exit(2)

    # Get the token
    token = get_token(roslyn_dir, 'git')
    if not token:
